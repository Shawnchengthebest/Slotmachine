...
<head>
    ...
    <style>
        * {
        ...
        .reel.spinning {
            animation: slotSpin 0.03s linear infinite;
        }
        ...
        @media (max-width: 1200px) {
            ...
            .admin-panel, .game-panel, .history-panel {
                min-width: auto;
            }
        }
...
<body>
    ...
    <script>
        ...
        function forceWin() {
            if (!isAdminLoggedIn) return;
            // This will be used in the spin function
        ...
        async function spin() {
            ...
            // Start spinning animation with smoother timing
            const spinDuration = 2500; // 2.5 seconds
            const frameDuration = 20; // 20ms per frame for 60fps
            const totalFrames = spinDuration / frameDuration;
            
            // Add spinning class to reels
            reels.forEach(reel => reel.classList.add('spinning'));
            
            // Pre-generate all symbols for smooth animation
            ...
            for (let frame = 0; frame < totalFrames; frame++) {
                // Update reels with pre-generated symbols
            ...
            // Generate final result (with admin controls)
            let finalResults;
            
            if (window.forceNextWin) {
                // Force a win with three matching symbols
                ...
                // Force a loss with no matching symbols
                finalResults = [
                ...
                // Normal spin with custom win rate
...
def create_and_open_html():
    """Create the HTML file and open it directly in the browser"""
    # Create HTML file
    ...
    file_path = os.path.abspath('slotmachine.html')
    
    # Open in browser
    webbrowser.open(f'file://{file_path}')
    
    print(f"✅ Slot machine opened! File location: {file_path}")
    print("💡 You can now run this script anytime to open the slot machine directly!")
